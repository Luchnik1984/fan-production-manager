...
INSERT INTO component (class_id, name, vendor_code, unit_id, description, weight_kg, material, created_at, created_by)
SELECT cl.id, 'Ступица SM1210', 'SM1210', u.id, 'Ступица для вала 20 мм', 0.45, 'Чугун СЧ20', NOW(), 'system'
FROM component_class cl, unit_of_measure u
WHERE cl.name = 'Ступицы с цилиндрической посадкой' AND u.code = 'шт';

INSERT INTO component (class_id, name, vendor_code, unit_id, description, weight_kg, material, created_at, created_by)
SELECT cl.id, 'Втулка 1210х24', 'BSH-1210-24', u.id, 'Коническая втулка диаметр 24 мм', 0.12, 'Сталь 45', NOW(), 'system'
FROM component_class cl, unit_of_measure u
WHERE cl.name = 'Втулки разрезные' AND u.code = 'шт';

INSERT INTO component (class_id, name, vendor_code, unit_id, description, technical_specs, material, created_at, created_by)
