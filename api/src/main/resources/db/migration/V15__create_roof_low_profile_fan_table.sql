...
-- Создание таблицы для крышных низкопрофильных вентиляторов с мотор-колесом
...
CREATE TABLE IF NOT EXISTS roof_low_profile_fan (
    ...
    execution_type VARCHAR(20),                -- KpM, KpMS
    roof_size VARCHAR(50),                     -- типоразмер (40/31)
    climate_type VARCHAR(10) DEFAULT 'У1',

    -- Мотор-колесо
    motor_wheel_id BIGINT,

    -- Расчётные поля (из мотор-колеса)
    poles INTEGER,
    voltage INTEGER,
